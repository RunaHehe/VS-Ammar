...
function onCreatePost()
	for i = 0, getProperty('unspawnNotes.length') - 1 do
		...
		if getPropertyFromGroup('unspawnNotes', i, 'noteType') == 'SwicthYtrolled' then
			if not sus then
...
function onUpdatePost(el)
    ...
    for a = 0, getProperty('notes.length') - 1 do
        ...
        if getPropertyFromGroup('notes', a, 'noteType') == 'SwicthYtrolled' then
			if sus then
				setPropertyFromGroup('notes', a, 'offsetY', getPropertyFromGroup('notes', a, 'offsetY') + 2 * math.cos((currentBeat + a * 0.15) * math.pi))
			end
			if (strumTime - songPos) < 950 / scrollSpeed and not sus then
			...
			elseif (strumTime - songPos) < 850 / scrollSpeed and sus then
