...
local immortal = false--not die if miss the punch
function onCreate()
    for punchs = 1,3 do--precache punchs
        createPunch(punchs)
        cancelTween('VoiidPunch'..punchs..'Alpha')
        setProperty('VoiidPunch'..punchs..'.alpha',0.001)
    end
    createWhiteMatt()
...
function createWhiteMatt(type)
	...
	makeAnimatedLuaSprite(name,'characters/WhiteMattStand_Attack',x,getProperty('boyfriend.y') - getProperty('boyfriend.positionArray[1]') + 250 + math.random(-10,10))
	scaleObject(name,1.5,1.5)
	setProperty(name..'.flipX',flipMatt)
	addAnimationByPrefix(name,'anim',anim,24,false)
	playAnim(name,'anim',true)
	setObjectOrder(name,getObjectOrder('boyfriendGroup') + (#whiteMatts + 1))
    ...
    if #punchEvents > 0 then
        for events = 1,#punchEvents do
            ...
            elseif event == 'punchMatt' then
                if strumTime <= 300 then
                    createWhiteMatt()
                    local punchs = tonumber(punchEvent[3])
                    if tonumber(punchs) ~= nil and tonumber(punchs) > 1 then
                        for punch = 2,punchs do
                            runTimer('createmattattackpunch'..punch,(stepCrochet*0.001 * (16/(punch+1))) + 0.1)
                        end
                    end
...
function createPunch(id,speed)
    ...
    if not luaSpriteExists(name) then
        ...
        makeAnimatedLuaSprite(name,'mechanics/punch'..id,0,0)
...
function removePunch(id)
    if luaSpriteExits('VoiidPunch'..id) then
        removeLuaSprite('VoiidPunch'..id,true)
    end
    if luaTextExists('VoiidPunchText'..id) then
        removeLuaText('VoiidPunchText'..id,true)
    end
end
    ...
    if name == 'punch' then
        ...
        if punch > 1 then
            for punchs = 1,punch-1 do
                local time = stepCrochet*0.001* (16/(punchs+1))
